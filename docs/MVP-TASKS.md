...
>
> **当前进度**：Sprint 2（项目管理）——Sprint 0、Sprint 1 已完成 ✅
...
**遗留 TODO**：freeze 时生成评分快照（Sprint 8）；publish 校验问卷已配置（Sprint 3）、被评人已配置（Sprint 4）。

---

## Sprint 3：问卷（数据结构 + Excel 导入）

- [ ] `Questionnaire` / `Dimension` / `Question` 模型 + migration
- [ ] 维度树：最大两级；校验"同一一级维度不能同时直接挂题目和二级维度"
- [ ] 题型：RATING（固定 10 档，必答）/ TEXT（可配置必填/选填，weight=null）
- [ ] 关系适用规则：维度级 4 个布尔（自评/上级/平级/下级）+ 题目级 override
- [ ] 问卷 Excel 模板生成（`scripts/` 或 API 下载）：16 个标准字段
- [ ] 问卷 Excel 导入：解析 → 校验 → 错误报告 → 事务写入
- [ ] 权重校验：一级维度合计=100%；同维度下二级维度合计=100%；同维度下题目合计=100%
- [ ] `POST /api/questionnaire/validate` 完整性校验接口
- [ ] 问卷锁定逻辑：首次正式提交后 lockedAt，禁止修改
- [ ] 问卷只读预览页（HR 端，按关系视角预览适用题目）
- [ ] 问卷模板：isTemplate 标记 + 从模板复制（简单版，无版本管理）

**验收**：用 PRD 第 12.2 节字段的 Excel 可导入生成问卷；权重非法时导入/发布被拦截并给出错误明细。
