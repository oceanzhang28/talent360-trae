...
## Sprint 14：人员主数据回填 ✅（2026-09-16 完成）

目标（C 组数据质量项）：关系导入时把 Sprint 10 建好的全局人员主数据（`User.department/position/grade`）补进项目人员快照，解决两个真实痛点：

1. PRD 16.1 的 Excel 只有**被评人**的部门/岗位/职级，**评价人的部门在 Excel 里根本没有** → 此前评价人快照的部门恒为空，进度看板/关系列表显示不出评价人部门
2. 被评人三字段原本硬性必填，跨项目重复填写、容易不一致

- [x] **补全口径**（`modules/review-relations/validate.ts` 纯函数，主数据以入参注入）：
  - 被评人：Excel 显式填写**优先**；留空则取人员主数据；两处都没有才按 PRD 16.1 报错（错误提示改为「被评人部门为空（Excel 与人员主数据均无）」）
  - 评价人：Excel 无这些列，部门/岗位/职级**全部取自**人员主数据
  - 新增返回字段 `masterFilled`：统计有多少人的信息来自主数据补全，在导入预览区展示
- [x] **写入时一次性补全，不破坏快照语义**（铁律 6）：`previewRelationsImport` / `commitRelationsImport` 共用同一套 `checkImportedRelations`（保证 preview 与 commit 口径一致）；补全只发生在导入那一刻，之后在用户管理里改主数据不会回溯改动已生成的项目快照
- [x] **手工新增关系同样补全**（`createRelation`）：HR 手工加关系时无需再手填部门/岗位/职级
- [x] Excel 模板「填写说明」与导入页文案同步更新（说明留空会如何补全）

**关键设计判断**：
1. 补全逻辑放在纯函数里、主数据用 `Map` 入参注入，而不是在函数内部查库 —— 这样规则可被单测穷举，也保证 preview/commit 完全同口径。
2. 仍保留"必填"语义（两处都没有就报错），不静默放过：PRD 16.1 把这三个字段列为必填，放宽会让报告里出现无部门人员。

**验收结果**：✅ 单测新增 5 用例（留空由主数据补全 / Excel 优先于主数据 / 评价人取自主数据 / 两处均无则报错 / 不传主数据时保持原必填行为）+ 集成测试新增 5 用例（Preview 补全且不写库 / Commit 落库含评价人部门 / 快照语义：改主数据不影响已有快照、重新导入才更新 / 留空且无主数据 → 400 且不落库 / 手工新增关系同样补全）。全套 **250 vitest + 47 E2E** 通过，typecheck / lint / format 通过。

## Sprint 15：回收站与恢复 ✅（2026-09-16 完成）

目标（V2 第 1/2 项 + PRD 第 44/45 节）：补齐软删除的后续链路。此前只做了「打标记」（Sprint 2 遗留），**没有恢复、没有彻底清理、删除也没写审计**。

- [x] **删除补审计**（PRD 45「删除项目」）：`deleteProject` 写 `DELETE_PROJECT` 审计，记录项目名、删除前状态、保留期截止时间
- [x] **记录删除前状态**（`Project.statusBeforeDelete`，migration `add_project_status_before_delete`）：恢复时还原到删除前状态，而不是一律回落草稿
- [x] **回收站列表** `GET /api/projects/recycle-bin`（仅系统管理员）：删除前状态、删除时间、保留剩余天数、是否可彻底清理
- [x] **恢复** `POST /api/projects/:id/restore`：还原删除前状态 + 清空 `deletedAt/purgeAfter`，写 `RESTORE_PROJECT` 审计；恢复后做一次惰性状态同步（例如删除前是 ACTIVE 但截止时间已过 → 自动落 CLOSED）；历史数据无 `statusBeforeDelete` 时回落草稿
- [x] **彻底清理**（PRD 44「超过 30 天可彻底清理」）：
  - 单个：`DELETE /api/projects/:id/purge`，未过保留期 409（提示剩余天数），物理删除并级联清理问卷/维度/题目/人员/关系/任务/结果快照
  - 批量：`POST /api/projects/recycle-bin/purge-expired` 一键清理所有已过期项目
  - 审计先写入（`AuditLog.projectId` 是普通字段，不随项目删除消失，可留痕 `purged: true`）
- [x] **UI** `app/projects/recycle-bin`：回收站表格（删除前状态 / 删除时间 / 剩余天数 / 恢复 / 彻底清理）+ 顶部「清理已过期（N）」；项目列表页为系统管理员增加「回收站」入口

**关键设计判断**：
1. 回收站查询**不能复用** `requireProjectAdmin` —— 它带 `deletedAt: null` 过滤，会把回收站项目当作 404；因此单独写 `requireDeletedProject`（仅系统管理员）
2. 彻底清理是**不可逆**操作，与删除分开成两个动作：删除只是打标记（可恢复），清理才物理删除，且必须过 30 天；UI 上未到期直接禁用按钮并给出剩余天数
3. 保留期判定放在服务端（`purgeAfter` 与当前时间比较），前端禁用只是体验，不是鉴权

**验收结果**：✅ 集成测试新增 5 用例（回收站列表权限与剩余天数 / 恢复还原 PUBLISHED 状态 + 审计 + 已在站外再恢复 404 / 历史数据状态缺失回落草稿 / 清理：未到期 409 + 非管理员 403 + 到期后物理删除且级联五类数据清零 + 审计留痕 / 一键清理只清过期项目）+ E2E `tests/e2e/recycle-bin.spec.ts`（删除→列表不可见→回收站可见（草稿/剩余天数/清理按钮禁用）→恢复→列表重新可见；普通用户页面被重定向且接口 403，双工程）。全套 **255 vitest + 50 E2E** 通过，typecheck / lint / format 通过。
